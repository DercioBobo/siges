...
function set_queries(frm) {
	frm.set_query("student", () => ({ filters: { is_active: 1 } }));
...
function _render_cards(frm, groups) {
	...
	wrapper.html(`
		<style>
			...
			.turma-card-badge {
				...
				font-size: 10px; background: var(--red-light);
				color: var(--red); padding: 1px 6px; border-radius: 4px;
			}
		</style>
	...
	wrapper.find(".turma-card:not(.full)").on("click", function () {
		...
		frm.set_value("class_group", name);
	});
}
...
function highlight_selected_card(frm) {
	...
	wrapper.find(".turma-card").each(function () {
		const is_sel = $(this).data("name") === frm.doc.class_group;
