...
class MapaAproveitamento {
    ...
    _inject_styles() {
        ...
        s.textContent = `
            ...
            .ma-grid tr.absent td { color:#CBD5E1; }
            .ma-grid tr.absent td.computed { color:#CBD5E1; }
            @media print {
    ...
    _build_grid_html(subj, students) {
        ...
        const rows_html = students.map((s, i) => {
            ...
            return `
                <tr data-student="${frappe.utils.escape_html(s.student)}"${cls}>
                    ...
                    <td><input type="checkbox" class="ma-absent-chk" ${abs ? "checked" : ""}></td>
                </tr>`;
        }).join("");
            ...
            <table class="ma-grid">
                <thead>
                    <tr>
                        ...
                        <th title="Avaliação Contínua Sistemática Prática 3">ACSP 3</th>
                        <th title="Avaliação Contínua Sistemática Escrita 1">ACSE 1</th>
                        <th title="Avaliação Contínua Sistemática Escrita 2">ACSE 2</th>
                        <th title="Avaliação Contínua Sistemática Escrita 3">ACSE 3</th>
    ...
    _setup_grid_events(idx) {
        ...
        $area.find(".ma-absent-chk").on("change", (e) => {
            const $tr  = $(e.target).closest("tr");
            const abs  = e.target.checked;
            $tr.toggleClass("absent", abs);
            $tr.find(".ma-score").prop("disabled", abs).val("");
            $tr.find("[data-field='macsp'],[data-field='macs'],[data-field='mt']").text("");
            this._mark_dirty(idx);
            this._update_footer(idx);
        });
    ...
    _recalc_row($tr) {
        ...
        const acsp = ["acsp_1","acsp_2","acsp_3"].map(_n).filter(v => v !== null);
        const macsp = acsp.length ? _r2(acsp.reduce((a,b)=>a+b,0)/acsp.length) : null;
        $tr.find("[data-field='macsp']").text(macsp !== null ? macsp.toFixed(2) : "");

        const acse = ["acse_1","acse_2","acse_3"].map(_n).filter(v => v !== null);
        const macs_in = (macsp !== null ? [macsp] : []).concat(acse);
        const macs = macs_in.length ? _r2(macs_in.reduce((a,b)=>a+b,0)/macs_in.length) : null;
        $tr.find("[data-field='macs']").text(macs !== null ? macs.toFixed(2) : "");

        const acp = _n("acp");
        const mt  = (macs !== null && acp !== null) ? _r2((2*macs + acp)/3) : null;
        $tr.find("[data-field='mt']").text(mt !== null ? mt.toFixed(2) : "");
    }
    ...
    _update_footer(idx) {
        ...
        this.$grid_area.find(".ma-grid tbody tr").each((_, tr) => {
            const $tr = $(tr);
            if ($tr.find(".ma-absent-chk").is(":checked")) { absent++; return; }
            const mt = parseFloat($tr.find("[data-field='mt']").text());
            if (!isNaN(mt)) { mt >= 10 ? approved++ : failed++; }
        });
        ...
        $stats.html(`
            ...
            <span style="font-size:13px;color:#9CA3AF;font-weight:700;">
                <i class="fa fa-minus-circle"></i>&nbsp;S/Nota: ${absent}
            </span>
    ...
    _collect_rows(idx) {
        ...
        this.$grid_area.find(".ma-grid tbody tr").each((i, tr) => {
            ...
            rows.push({
                student,
                is_absent: abs,
                acsp_1: abs ? null : _iv("acsp_1"),
                acsp_2: abs ? null : _iv("acsp_2"),
                acsp_3: abs ? null : _iv("acsp_3"),
                acse_1: abs ? null : _iv("acse_1"),
                acse_2: abs ? null : _iv("acse_2"),
                acse_3: abs ? null : _iv("acse_3"),
                acp:    abs ? null : _iv("acp"),
            });
        });
        return rows;
    }
    ...
    _save(idx) {
        ...
        frappe.call({
            ...
            callback: (r) => {
                ...
                // Update computed cells from saved data
                const by_student = {};
                ...
                this.$grid_area.find(".ma-grid tbody tr").each((i, tr) => {
                    ...
                    $tr.find("[data-field='mt']").text(_f2(sr.mt));
                });
